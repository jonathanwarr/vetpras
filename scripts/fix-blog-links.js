// Script to fix the blog post links
...
The pet affordability crisis isn't new—but it's accelerating. A [June 2025 Gallup/PetSmart Charities study](https://news.gallup.com/poll/691325/financial-barriers-keep-canadian-pets-away-vet.aspx) found that **50% of Canadian pet owners skipped or declined necessary vet care** in the past year. Only 28% were offered alternatives when they asked about cost. Only 21% were offered payment plans, despite 66% saying they'd use interest-free options.

Even more striking: According to a [September 2025 Hill's Pet Nutrition report](https://www.veterinarypracticenews.ca/report-canadian-pet-adoption/), 72% of prospective pet adopters cite veterinary care as a top 3 expense concern, and **72% say vet costs directly affect their willingness to adopt**.
...
1. **Use a tool like Vetpras to get a directional sense of costs based on real bills** submitted by other pet owners in your area.
...
async function updatePost() {
  const { data, error } = await supabase
    .from('blog_posts')
    .update({ content })
    .eq('slug', 'vancouver-vet-price-gap-explained')
    .select();
  ...
  console.log('✅ Blog post links updated successfully!');
  console.log('Post ID:', data[0].id);
  console.log('\nChanges made:');
  console.log('1. ✓ Updated Gallup link to actual URL');
  console.log('2. ✓ Updated Hill\'s Pet Nutrition link to actual URL');
  console.log('3. ✓ Updated Abacus Data link to actual URL');
  console.log('4. ✓ Removed link from "Vetpras" in How to Use section');
}
